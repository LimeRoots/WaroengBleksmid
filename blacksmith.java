        ...
        for (recipe resep : daftarResep) {
            if (resep != null && resep.ambilNamaResep().equals(namaResep)) {
                ...
                for (int i = 0; i < material.length; i++) {
                    Material mat = inventory.listMaterial.cariMaterial(material[i]);
                ...
                if (bisaMembuat) {
                    for (int i = 0; i < material.length; i++) {
                        Material mat = inventory.listMaterial.cariMaterial(material[i]);
                        if (mat != null) {
                            System.out.println("Memakai " + material[i] + " sebanyak " + (kuantitas[i] * jumlah));
                            inventory.kurangItemMaterial(material[i], kuantitas[i] * jumlah);
                        }
                    }
                    System.out.println("Mendapatkan " + jumlah + " " + namaResep);
